...
### Decision: FACT / INFERENCE / RECOMMENDATION / UNCERTAIN labelling on the patient report (for feature 3 / hard gate c, increment 3)

Hard gate (c): every AI-generated insight must be labelable on this four-way epistemic scale *with an explainability affordance*. The recommendation report already produces the four kinds of content separately — `signals` (observed), `deficiencies` (inferred, with a confidence number), `vitamins` (suggested protocol), `disclaimer` (caveat) — the pharmacist already reads them in `Fact → Inference → Recommendation` order (prd.md §6.4). So this is a labelling + disclosure layer, not new analysis.

`src/components/InsightLabel.tsx` is one reusable component: a colour-coded tag (`fact` teal / `inference` amber / `recommendation` green / `uncertain` grey) plus a "?" button that opens a plain-language note explaining what that label means and, where relevant, which source the data came from. Applied as a section-level label on each report card in `ClientDashboard`, with a plain-language summary paragraph (assembled from the report's own counts — nothing invented) above the cards.
