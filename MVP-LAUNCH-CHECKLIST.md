...
- [ ] Dataset ingestion (PRD §6.1: "ingest CSV/API exports"): the current "Register Dataset" modal (`src/pages/App/Datasets.tsx`) only captures a name + type — there's no actual file upload or CSV parsing yet. Decide if real ingestion is in MVP scope or if manual registration is acceptable for the pilot. **Note (2026-08-27):** the sample "Pharmaceutical Drugs and Vitamins" catalog data intended for this feature turned out to be an image dataset, not tabular product data — see `data/DATA_PROVENANCE.md`. A real catalog data source is still needed before this can be demoed with anything beyond the Supplement Sales sample data.
